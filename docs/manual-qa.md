...
| P-03 `release` publication | Protected `release` environment, protected version tag, maintainer approval, and all previous evidence | Push a tag exactly matching `v` plus the `Cargo.toml` version; do not run publication as manual QA. After approved publication, validate `cargo install dsql-cli --locked` only on a clean host. | The tagged workflow publishes crates.io and then creates the GitHub Release; record both links and first-run evidence. | Uninstall only on the clean-host test environment if policy requires. |
